...
class REMASTER(Optimizer):
    ...
    def step_parameter(self, p, g):
        ...
        bias_correction = 1 - group["betas"][0] ** step # Can apply to step_size, but this leads to significant initial updates and could be too much without warmup.
        bias_correction_sqrt = (1 - slow_beta ** step) ** (1 / 2)
        atan2_mul = 1.27323954474 # atan2(1,1) renormalization multiplier
        step_size = group["lr"] * atan2_mul
        ...
        if group["weight_decay"] != 0:
            ...
            full_step.add_(grad_weights, alpha=group["weight_decay"] * group["weight_decay_rate"]**step)
